...
`skill-io-contract` is a local-first checker for `SKILL.md` files and their fixture cases. It looks for the sections agents need in order to route a skill safely, then compares JSON fixture cases against a small contract schema. The default mode is read-only and emits a Markdown report.
...
- Required `SKILL.md` guidance for triggers, inputs, outputs, side effects, approvals, examples, and validation.
